// script.js — UI + auth + history + uploads + mic + Markdown + LIVE STREAM
const API_BASE = localStorage.getItem("ASKUNI_API") || "http://localhost:3000";
...
async function api(path, opts = {}) {
  ...
  if (res.status === 401) {
    ...
    if (AUTO_OPEN_LOGIN_ON_401) { try { loginModal.showModal(); } catch {} } else { promptLogin(msg); }
    const err = new Error(msg); err.code = 401; throw err;
  }
...
function addStreamingBubble() {
  ...
  return {
    append(piece) {
      full += piece;
      // show raw during stream for speed
...
function clearChat() {
  chatEl.innerHTML = `<div class="chat__empty" id="emptyState">
...
function updateUserUI() {
  ...
  msgInput.disabled = !loggedIn; sendBtn.disabled = !loggedIn; fileInput.disabled = !loggedIn; micBtn.disabled = !loggedIn;
}
...
fileInput.addEventListener("change", async () => {
  ...
  if (!state.token) { promptLogin("Sign in to attach files."); fileInput.value = ""; return; }
  const file = fileInput.files[0];
  const form = new FormData(); form.append("file", file);
  addMessage("bot", "📎 Uploading attachment…");
  try {
    const res = await fetch(`${API_BASE}/upload`, { method: "POST", headers: { "Authorization": "Bearer " + state.token }, body: form });
...
async function sendMessageStreaming(textOverride = null, opts = { alreadyEchoed: false }) {
  if (!state.token) { promptLogin("You need to sign in to send messages."); return; }
...
msgInput.addEventListener("keydown", (e) => {
  if ((e.ctrlKey || e.metaKey) && e.key === "Enter") sendMessageStreaming();
});

// -------- Mic (unchanged) --------
let mediaStream = null, recorder = null, chunks = [], isRecording = false, autoStopTimer = null;
async function startMic() {
  if (!state.token) { promptLogin("Sign in to use the microphone."); return; }
  try {
...
async function onMicStop() {
  try {
    ...
    const res = await fetch(`${API_BASE}/stt`, { method: "POST", headers: { "Authorization": "Bearer " + state.token }, body: form });
    const json = await res.json();
