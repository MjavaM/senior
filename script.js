...
// Same-origin by default (works locally + on Render). Override if you split frontend/backend:
...
// Main
const chatEl = q("#chat");
const emptyEl = q("#emptyState"); // may be null (we removed the empty state)
...
// prompt hint (only when we actually had a token)
...
(function init() {
  ...
  clearChat(); // start with a clean, full-height chat
