...
// Allow external background images via CSS (Unsplash/CDNs)
...
// ---------------- DB ----------------
if (!DATABASE_URL) {
  console.warn("[AskUni] WARNING: DATABASE_URL is not set — login/history endpoints will fail if called.");
}
const pool = new Pool({
...
try {
  ...
  console.warn("[AskUni] DB init failed (running without persistence?):", e.message || e);
}
...
app.post("/message", async (req, res) => {
  try {
    ...
    if (OPENAI_API_KEY && ASSISTANT_ID) {
      const threadId = isLoggedIn && DATABASE_URL
