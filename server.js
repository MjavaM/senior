// server.js — AskUni API (STRICT vector-store-only answers, streaming, STT, uploads, auth)
...
const VECTOR_STORE_ID = (process.env.ASKUNI_VECTOR_STORE_IDS || "")
  .split(",")
  .map(s => s.trim())
...
SOURCE OF TRUTH - MANDATORY SEARCH PROTOCOL:
- You MUST use the file_search tool for EVERY question - no exceptions.
- Search the vector store THOROUGHLY before responding:
  * Try multiple search queries with different keywords
  * Search for course codes (e.g., "ITCS285", "itcs 285", "ITCS-285")
  * Search for related terms (e.g., if asked about "instructor", also search "teacher", "professor", "faculty")
  * If first search fails, try broader or narrower terms
- ONLY after exhausting all search attempts, use the fallback message.
- If the vector store contains the answer, you MUST find it and cite it.
- NEVER generate answers from your training data - ONLY from vector store search results.
- NEVER guess, estimate, or infer information not explicitly in the vector store.
- NEVER make up instructor names, room numbers, schedules, or any other details.

RESPONSE RULES:
- Every answer MUST include file_citation annotations from the vector store.
- If you cannot find citations after thorough searching, respond with: "I don't have this information in my knowledge base. Please check SIS/UCS or upload the official document."
- When you find information, cite the specific file and provide detailed answers.

FORMAT (Markdown):
- **Answer** then **Overview**, **Key Details**, **Next Steps**.
- Use tables when showing multiple sections: | Section | Instructor | Days & Time | Location | Seats |
`.trim();
...
async function dbSetThreadId(sessionId, threadId) {
  await pool.query(`UPDATE chat_sessions SET thread_id=$2, updated_at=now() WHERE session_id=$1`, [
    sessionId,
    threadId,
  ]);
}
...
app.post("/auth/login", async (req, res) => {
  try {
    ...
    res.json({
      ok: true,
      user: { id: user.id, email: user.email, first_name: user.first_name, last_name: user.last_name, registered_at: user.registered_at },
      token,
    });
  } catch (e) {
...
app.post("/auth/request-reset", async (req, res) => {
  try {
    ...
    let devCode = null;
    try { await sendResetEmail(email, code); } catch { devCode = code; }
...
app.post("/auth/reset", async (req, res) => {
  try {
    ...
    if (!(newPassword?.length >= 8 && /[A-Za-z]/.test(newPassword) && /\d/.test(newPassword)))
      return res.status(400).json({ ok: false, error: "Password must be ≥8 chars & include letters & numbers." });
...
function hasKBAnnotations(message) {
  ...
  for (const part of message.content) {
    if (part.type === "text" && part.text?.annotations?.length) {
      ...
      // Check for file_citation which indicates vector store usage
...
/** Extract annotations from assistant message to verify vector store was used */
function getAnnotationCount(message) {
  if (!message?.content?.length) return 0;
  let count = 0;
  for (const part of message.content) {
    if (part.type === "text" && part.text?.annotations?.length) {
      count += part.text.annotations.filter(a => a?.type === "file_citation").length;
    }
  }
  return count;
}
...
// ------------------------- Core: Non-streaming (citation required) -------------------------
app.post("/message", async (req, res) => {
  try {
    ...
    if (!OPENAI_API_KEY || !ASSISTANT_ID)
      return res.status(503).json({ ok: false, error: "Assistant offline (missing OPENAI_API_KEY or ASKUNI_ASSISTANT_ID)" });
    if (!VECTOR_STORE_ID)
      return res.status(503).json({ ok: false, error: "Knowledge base not attached (ASKUNI_VECTOR_STORE_IDS missing)" });
    ...
    const threadId = isLoggedIn && DATABASE_URL ? await ensureThreadForSession(sid) : (await createGuestThread()).id;
    ...
    const run = await openai.beta.threads.runs.create(threadId, {
      ...
      // IMPORTANT: only file_search tool allowed with max search configuration
      tools: [{ type: "file_search" }],
      // Temperature 0 for most factual responses
      temperature: 0,
    });
    ...
    // Read latest assistant message and enforce citations
    const { data } = await openai.beta.threads.messages.list(threadId, { limit: 5 });
    const latest = data.find((m) => m.role === "assistant");

    const citationCount = getAnnotationCount(latest);
    let text = "";
    
    if (hasKBAnnotations(latest) && citationCount > 0) {
      console.log(`[AskUni] Non-stream response validated with ${citationCount} vector store citation(s)`);
      text = (latest?.content || [])
        .map((c) => (c.type === "text" ? c.text.value : ""))
        .join("\n")
        .trim();
    } else {
      console.warn(`[AskUni] Non-stream response without vector store citations. Citations: ${citationCount}`);
...
// ------------------------- Core: Streaming (buffers, then citation-checks) -------------------------
app.post("/message/stream", async (req, res) => {
  ...
  try {
    ...
    if (!OPENAI_API_KEY || !ASSISTANT_ID) {
      ...
      send("final", { text: "Assistant offline.", sessionId: sessionId || null });
      return res.end();
    }
    if (!VECTOR_STORE_ID) {
      res.setHeader("Content-Type", "text/event-stream; charset=utf-8");
      send("delta", { t: "ℹ️ Knowledge base not attached (ASKUNI_VECTOR_STORE_IDS missing)." });
      send("final", { text: "Knowledge base missing.", sessionId: sessionId || null });
      return res.end();
    }
    ...
    stream.on("end", async () => {
      ...
      // CRITICAL: Fetch the final assistant message to verify vector store was actually used
      try {
        const { data } = await openai.beta.threads.messages.list(threadId, { limit: 5 });
        const latest = data.find((m) => m.role === "assistant");
        
        // Log the full message for debugging
        console.log("[AskUni] Full assistant response:", JSON.stringify(latest, null, 2));
        
        // Count citations to verify vector store was searched
        const citationCount = getAnnotationCount(latest);
        
        // If no citations from vector store, the assistant did NOT search properly
        if (!hasKBAnnotations(latest) || citationCount === 0) {
          console.warn(`[AskUni] ⚠️  Response generated without vector store citations. Citations found: ${citationCount}`);
          console.warn(`[AskUni] Original response was: ${full.slice(0, 200)}...`);
          full = "**Answer**\n\nI don't have this information in my knowledge base.\n\n**Next Steps**\n- Please check SIS/UCS or upload the official document.\n- Make sure the information you're looking for is in the uploaded course materials.";
        } else {
          console.log(`[AskUni] ✅ Response validated with ${citationCount} vector store citation(s)`);
        }
      } catch (err) {
      ...
      if (isLoggedIn && DATABASE_URL) {
        try { await dbAddMessage(sid, "bot", full.trim(), []); } catch {}
      }
...
app.post("/stt", upload.single("audio"), async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ ok: false, error: "STT failed", detail: e?.message || String(e) });
  } finally {
